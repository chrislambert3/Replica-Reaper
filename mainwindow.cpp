...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(watcher, &QFileSystemWatcher::directoryChanged, this,
          [=](const QString &path) {
            ...
            debounceTimer->setProperty("changedPath",
                                       path);  // store changed path
          });
  ...
  connect(debounceTimer, &QTimer::timeout, this, [=]() {
    ...
    if (pathToTypeMap.find(changedPath) != pathToTypeMap.end()) {
      ...
      FileManager::FileOrDownloads dirType =
          pathToTypeMap.find(changedPath)->second;
      ...
      if (!fileList.isEmpty()) {
        ...
        if (lastFile.suffix() != "tmp") {
          ...
          for (const auto &otherFileInfo : fileList) {
            if (lastFile.absoluteFilePath() ==
                otherFileInfo.absoluteFilePath()) {
            ...
            if (lastFile.size() == otherFileInfo.size() &&
              ...
              fs::path lastFPath =
                  otherFileInfo.absoluteFilePath().toStdString();
              FileInfo lastFile(
                  lastFPath,
                  QString::fromStdString(lastFPath.extension().string()),
                  fs::file_size(lastFPath));
              fs::path otherFPath =
                  otherFileInfo.absoluteFilePath().toStdString();
              FileInfo otherFile(
                  otherFPath,
                  QString::fromStdString(otherFPath.extension().string()),
                  fs::file_size(otherFPath));
          ...
          FileInfo file(fPath,
                        QString::fromStdString(fPath.extension().string()),
                        fs::file_size(fPath));
          ...
          if (manager->isDupe(file, dirType)) {
            ShowNotification(
                "Duplicate Detected, Open ReplicaReaper to delete: ",
                file.getFileName());
            this->mode = dirType;
          }
        }
...
void MainWindow::onReaperButtonClicked() {
  ...
  QString path;
  if (this->mode == FileManager::Files) {
    path = manager->PromptDirectory(this);
    if (path == QString()) {
  ...
  for (int i = 0; i < filePaths.size(); ++i) {
    ...
    if (this->getCancelButtonState()) {
      ...
      ShowNotification("Reaping Canceled",
                       "Scanning has cancelled sucessfully");
      return;
    }
    ...
    manager->addFileToTypeSizeMap(file,
                                  FileManager::Files);  // passes in fileinfo
  ...
  ui->HowUseBTN->setEnabled(true);
  // reset the mode
  mode = FileManager::Files;
...
void MainWindow::ShowDupesInUI(const FileManager &f,
                               FileManager::FileOrDownloads choice) {
  ui->treeWidget->blockSignals(true);
  // determine which map to iterate over
  // const auto& DupesMap = (choice == FileManager::Files) ? f.Dupes :
  // f.DownloadDupes;
  ...
  switch (choice) {
    ...
    case FileManager::Downloads:
      DupesMap =
          &f.DownloadDupes;  // Assign f.DownloadDupes map for Download case
      break;
    case FileManager::Documents:
  ...
  for (auto it = DupesMap->begin(); it != DupesMap->end(); ++it) {
    ...
    parentHashItem->setToolTip(
        1, QString::fromStdString(it->second.front().getFilePath().string()));
  ...
  connect(ui->treeWidget->header(), &QHeaderView::sectionResized, this,
  ...
  connect(ui->treeWidget, &QTreeWidget::itemExpanded, this,
          [=](QTreeWidgetItem *) { adjustColumnWidth(); });
...
void MainWindow::showDeleteConfirmation(
  ...
  if (originalCount > 0) {
    ...
    label->setText(label->text() +
                   "<br><b>You have: " + QString::number(originalCount) +
                   " original files selected. </b>");
  }
...
void MainWindow::onTrayIconActivated(QSystemTrayIcon::ActivationReason reason) {
  ...
  if (reason == QSystemTrayIcon::Trigger && Qui) {
    ...
    if (Qui->isHidden()) {
      ...
      // If background detection is not enabled (Files Mode) dont show found
      // duplicates
      if (this->mode != FileManager::Files) {
